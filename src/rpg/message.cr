module RPG
  class Message
    ...
    # TODO: make this more generic with more options and move into GSF
    def initialize(@cx, @y, @max_width, @message = "", @typing = false, @animate = false)
      ...
      @typing_timer = Timer.new(@message.empty? ? TypeDuration : TypeDuration * @message.size)
    ...
    def update(keys : Keys)
      ...
      if keys.just_pressed?([Keys::Enter, Keys::Space, Keys::E])
        if @typing_timer.done?
          ...
          @typing_timer.duration = TypeDuration
        end
      end
    end
